...
#global constants
  ...
  rm(df)
  one_frame = subset(match,frame == 100)
  scale = 8.2/0.6
  x_segmentation = 20
  y_segmentation = 14
  pitch_length = 107
...
plot_pc <- function(pc,frame){
  return(soccerPitch() +
    scale_color_manual(values=c(pal[2],pal[4]))+
    scale_fill_manual(values=c(pal[2],pal[4]))+
    labs(x="",y="")+
    geom_sf(data=pc,mapping = aes(fill=as.factor(team)),alpha=0.2,show.legend = F)+
...
p1 <- one_frame %>% 
  calculate_pitch_control(fujsug) %>%
  plot_pc(one_frame)
p2 <- one_frame %>% 
  calculate_pitch_control(taki) %>%
  plot_pc(one_frame)
...
calc_brefeld <- function(this.times = times,this.speeds = speeds,this.teams = teams,
                         path = "C:/Users/David/OneDrive/Documents/Work/Thesis/Code/Data/test/transformed_points"){
  for (team in this.teams){
    for (player in 1:11){
      for (speed in this.speeds){
        print(paste0(team,"/player_",player,"/speed_",speed))
        get_kde <- function(time){
          print(time)
          directory <- paste0(path,"/",team,"/player_",player,"/speed_",speed,"/time_",time,".csv.gz")
          trans_points <- read.csv(directory)
          dens <- kde2d(trans_points$X0,trans_points$X1,
                        h = c(ifelse(bandwidth.nrd(trans_points$X0) < 0.001, 0.1, bandwidth.nrd(trans_points$X0)),
                              ifelse(bandwidth.nrd(trans_points$X1) < 0.001, 0.1, bandwidth.nrd(trans_points$X1))),
        ...
        speed <- speed(speed,lapply(FUN=get_kde,X=seq(1,length(this.times))))
        save(speed,file=paste0(path,"/",team,"/player_",player,"/speed_",speed,"/","speed.RData"))
        rm(speed)
      }
    }
  }
...
cl <- makeCluster(4)
registerDoParallel(cl)
test_partial <- foreach(i = 1:length(times),
                     .combine="c",
                     .packages = c('MASS','dplyr','sf','igraph'),
                     .export = ls(globalenv())) %dopar% partial(times[i],
                                                                one_frame,c("Gliders2016","Helios2016"))
stopCluster(cl)

test <- one_frame %>%
  calculate_pitch_control(c("Gliders2016","Helios2016"))
...
soccerPitch() +
  ...
  #coord_fixed()+
  labs(x="",y="")+
  geom_sf(data=voronoi,mapping = aes(fill=as.factor(team)),alpha=0.2,show.legend = F)+
...
con = unz("C:/Users/David/OneDrive/Documents/Work/Thesis/Code/Data/matches_formatted.zip",
                     "matches_formatted/cyrus2017-vs-Gliders2016/17-20170905234225-CYRUS_0-vs-Gliders2016_0.csv")
match <- read.csv(con)
close(con)
one_frame = subset(match,frame == 4000)
colfunc <- colorRampPalette(c(pal[4],pal[3],"white",pal[1],pal[2]))
soccerPitch() +
  scale_color_manual(values=c(pal[5],pal[2],pal[4]))+
  scale_fill_gradientn(colours=c(pal[4],pal[3],"white",pal[1],pal[2]),
                       values = c(0,0.49,0.5,0.51,1),
                       limits = c(0,1))+
  coord_fixed()+
  labs(x="",y="")+
  #geom_contour_filled(data = calculate_pitch_control(one_frame,"ppc") %>% reshape2::melt(),
  #             mapping = aes((Var1-0.5)*x_bin_width,(Var2-0.5)*y_bin_width,z=value,fill=..level..)) +
  geom_raster(data = calculate_pitch_control(one_frame,"ppc") %>% reshape2::melt(),
...
match = read.csv("C:/Users/David/Desktop/test.csv") %>% subset(frame %in% seq(100,601))
p <- soccerPitch() +
  #fte_theme()+
  scale_color_manual(values=c(pal[5],pal[2],pal[4]))+
  scale_fill_manual(values=c(pal[9],pal[2],pal[4]))+
...
anim <- animate(p, fps = 10,nframes=501)
anim
