...
def computeDistances(trajs, simpTraj, pth, r, possStarts, possEnds, upth, pathDic):
    ...
    for i in range(len(pth.indices)):
        currentIndex = int(pth.indices[i])
...
def process(trajs, rmin, rmax):
    ...
    while r <= rmax:
        ...
        for pth in pathlets:
            ...
            startFNeighbors = findNeighboringPts(fGrid, trajs[pth.trajID].pts[int(pth.bounds[0])])
            endFNeighbors = findNeighboringPts(fGrid, trajs[pth.trajID].pts[int(pth.bounds[1])])
            ...
            startBNeighbors = findNeighboringPts(bGrid, trajs[pth.trajID].pts[int(pth.bounds[0])])
            endBNeighbors = findNeighboringPts(bGrid, trajs[pth.trajID].pts[int(pth.bounds[1])])
